...
(int64) encodings, and primitive columns (int64, float64, string/UTF8) with
optional nulls. Column statistics
...
zlib and can be selected with `CPANDAS_PARQUET_CODEC=gzip`. Nested schemas and
complex types are not supported yet. The default row-group size is 65,536 rows.
