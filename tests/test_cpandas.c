...
static void test_json_io(void) {
  ...
  CHECK(path != NULL);
  if (path) {
    CHECK(cp_df_write_json(df, path, &err));
    char *contents = read_file(path);
    CHECK(contents != NULL);
    const char *expected =
        "[{\"id\":1,\"score\":2.5,\"name\":\"Alice\"},"
        "{\"id\":2,\"score\":null,\"name\":\"Bob\"},"
        "{\"id\":null,\"score\":4,\"name\":null}]\n";
    if (contents) {
      CHECK(strcmp(contents, expected) == 0);
    }
    free(contents);
    ...
    cp_error_clear(&err);
    CpDataFrame *df2 = cp_df_read_json(path, dtypes, 3, &err);
    CHECK(df2 != NULL);
    if (df2) {
  ...
  const char *json =
      "[{\"id\":1,\"score\":2.5,\"name\":\"Alice\"},"
      "{\"id\":2,\"name\":\"Bob\"},"
      "{\"id\":3,\"score\":true,\"name\":\"Dana\"}]";
  CHECK(write_file(path2, json));
  cp_error_clear(&err);
  CpDataFrame *df3 = cp_df_read_json(path2, dtypes, 3, &err);
  CHECK(df3 != NULL);
  if (df3) {
    const CpSeries *score = cp_df_get_col(df3, "score");
    CHECK(score != NULL);
    double score_val = 0.0;
    int is_null = 0;
    CHECK(cp_series_get_float64(score, 1, &score_val, &is_null));
...
static void test_ndjson_io(void) {
  CpError err;
  cp_error_clear(&err);
  ...
  CHECK(path != NULL);
  if (path) {
    CHECK(cp_df_write_ndjson(df, path, &err));
    char *contents = read_file(path);
    CHECK(contents != NULL);
    const char *expected =
        "{\"id\":1,\"score\":2.5,\"name\":\"Alice\"}\n"
    ...
    cp_error_clear(&err);
    CpDataFrame *df2 = cp_df_read_ndjson(path, dtypes, 3, &err);
    CHECK(df2 != NULL);
    if (df2) {
      const CpSeries *id = cp_df_get_col(df2, "id");
      const CpSeries *score = cp_df_get_col(df2, "score");
  ...
  const char *ndjson =
      ...
      "{\"id\":2,\"name\":\"Bob\"}\n"
      "{\"id\":3,\"score\":true,\"name\":\"Dana\"}\n";
  CHECK(write_file(path2, ndjson));
  cp_error_clear(&err);
  CpDataFrame *df3 = cp_df_read_ndjson(path2, dtypes, 3, &err);
  CHECK(df3 != NULL);
  if (df3) {
    const CpSeries *score = cp_df_get_col(df3, "score");
    CHECK(score != NULL);
...
static void test_parquet_stub(void) {
  ...
  if (path) {
    ...
    cp_error_clear(&err);
    CHECK(cp_df_read_parquet(path, &err) == NULL);
    CHECK(err.code == CP_ERR_INVALID);
    remove(path);
    free(path);
  }
